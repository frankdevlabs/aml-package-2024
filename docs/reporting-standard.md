...
1. `# Status snapshot — EU AML Package — AMLR/AMLA/AMLD6` + the **dashboard table** + the not-legal-advice blockquote
2. `## One-line status`
...
analysis page, and end each substantive cell with a deep link into the operative text under
[`extracts/amla/`](../extracts/amla/) (prefer a stable explicit `<a id=…>` anchor over a heading slug).
...
| New / updated AMLA draft instrument (RTS/ITS/GL) | dashboard **Latest live workstream**; refresh the **AMLA pipeline** table | [`docs/amla-pipeline.md`](amla-pipeline.md), [`positions.csv`](../data/positions.csv), [`extracts/amla/`](../extracts/amla/) |
